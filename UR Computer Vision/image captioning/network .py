...
class LSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.ini = nn.Linear(input_size, hidden_size * 4) 
        self.inh = nn.Linear(hidden_size, hidden_size * 4) 
    ...
    def forward(self, x, hidden):
        ...
        gi, gf, gc, go =( self.ini(x) + self.inh(hx)).chunk(4, 1)

        cy = (torch.sigmoid(gf) * cx) + (torch.sigmoid(gi) * torch.tanh(gc))
        hy = torch.sigmoid(go) * torch.tanh(cy)
        hiddenout = (hy, cy)
...
#         self.lstm = nn.LSTM(embed_size, hidden_size, num_layers, batch_first=True) 
    ...
    def forward(self, features, captions, lengths):
        ...
        emb1 = self.embed(captions)
        feat1=(torch.unsqueeze(features,1))
        inputsx = torch.cat((feat1, emb1), 1)
        xprime, xlast = self.lstm(inputsx, states)

        outputs = self.linear(xprime)    # outputs: (batch_size, t+1, vocab_size)
